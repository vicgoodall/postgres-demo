...
#opens an actual session by calling the Sessions() subclass defined above
session = Session()

#create db using the declarative_base subclass
...
#query2 - select all names from the 'Artist' table
#artists = session.query(Artist)
#for artist in artists:
#    print(artist.Name)

#query3 - select records with name = Queen
#artist = session.query(Artist).filter_by(Name="Queen").first()
#print(artist.ArtistId, artist.Name, sep=" | ")

#query4 - select only by Arist Id 51 from Artist table
#artist = session.query(Artist).filter_by(ArtistId=51).first()
#print(artist.ArtistId, artist.Name, sep=" | ")

#query5 - select only the albums with Artist Id 51 on the Album table
albums = session.query(Album).filter_by(ArtistId=51)
for album in albums:
    print(album.AlbumId, album.Title, sep=" | ")

#query 6 - all tracks in Track table with composer listed as Queen
